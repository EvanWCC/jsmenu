...
const breakfast = menu.filter(p => p.category === "breakfast");
const lunch = menu.filter(p => p.category === "lunch");
const shakes = menu.filter(p => p.category === "shakes");
const dinner = menu.filter(p => p.category === "dinner");
...
const displayItems = (menu) => {
  ...
  all = all.join(""); //you can add or not to add
  
  cards.innerHTML = all
}

allBtn.addEventListener('click',displayAll)
breakfastBtn.addEventListener('click',displayBreakfast)
lunchBtn.addEventListener('click',displayLunch)
shakesBtn.addEventListener('click',displayShakes)
dinnerBtn.addEventListener('click',displayDinner)
 
document.body.onload(displayAll()); //default load all
...
function displayBreakfast () {
  return displayItems(breakfast)
}

function displayLunch () {
  return displayItems(lunch)
}

function displayShakes () {
  return displayItems(shakes)
}

function displayDinner () {
  return displayItems(dinner)
}
